...
func TestRegisterRoutesIssuesTicket(t *testing.T) {
	...
	store, err := redisstore.NewRedisStore(client, "sso")
	if err != nil {
		t.Fatalf("expected store creation success, got %v", err)
	}
	service := application.NewService(store, domain.Config{DefaultTTLSeconds: 300, MaxTTLSeconds: 1800, KeyPrefix: "sso"})
...
func (issuer *stubIssuer) Issue(_ context.Context, request domain.IssueRequest) (domain.IssueResult, error) {
	if request.UserID <= 0 {
		return domain.IssueResult{}, fmt.Errorf("user id must be positive")
	}
	return domain.IssueResult{Ticket: "t", ExpiresAt: time.Unix(1, 0)}, nil
}
