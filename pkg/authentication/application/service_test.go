...
func TestServiceIssueStoresTicketWithDefaultTTL(t *testing.T) {
	...
	result, err := service.Issue(context.Background(), domain.IssueRequest{UserID: 10})
	if err != nil {
...
func TestServiceIssueRejectsInvalidInputs(t *testing.T) {
	service := NewService(&stubStore{}, domain.Config{DefaultTTLSeconds: 300, MaxTTLSeconds: 1800})
	if _, err := service.Issue(context.Background(), domain.IssueRequest{UserID: 0}); err == nil {
		t.Fatalf("expected issue failure for invalid user id")
	}
	if _, err := service.Issue(context.Background(), domain.IssueRequest{UserID: 1, TTL: 31 * time.Minute}); err == nil {
