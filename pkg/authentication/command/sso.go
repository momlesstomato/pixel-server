...
func ExecuteSSO(options Options, output io.Writer) error {
	...
	store, err := authentication.NewRedisStore(client, loaded.Authentication.KeyPrefix)
	if err != nil {
		return err
	}
	service := authentication.NewService(store, loaded.Authentication)
	result, err := service.Issue(context.Background(), authentication.IssueRequest{
