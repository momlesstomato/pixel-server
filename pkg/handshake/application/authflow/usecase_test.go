...
func TestAuthenticateUseCaseKicksDuplicateSession(t *testing.T) {
	...
	if len(transport.sent) == 0 || transport.sent[0] != packetauth.DisconnectReasonPacketID {
		t.Fatalf("expected disconnect_reason packet before duplicate close, got %v", transport.sent)
	}
}
...
func TestAuthenticateUseCaseRejectsInvalidInputs(t *testing.T) {
	...
	if len(transport.sent) == 0 || transport.sent[0] != packetauth.DisconnectReasonPacketID {
		t.Fatalf("expected disconnect_reason packet before unauthorized close, got %v", transport.sent)
	}
}
