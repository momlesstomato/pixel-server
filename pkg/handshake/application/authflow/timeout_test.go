...
type timeoutTransportStub struct {
	// sent stores packet identifiers sent by timeout flow.
	sent []uint16
...
func (stub *timeoutTransportStub) Send(_ string, packetID uint16, _ []byte) error {
...
func (stub *timeoutTransportStub) CloseWithProtocolReason(connID string, protocolReason int32, _ int, _ string) error {
	if protocolReason != 0 {
...
func TestTimeoutUseCaseWaitClosesConnectionOnTimeout(t *testing.T) {
	...
	if len(transport.sent) != 1 || transport.sent[0] != packetauth.DisconnectReasonPacketID {
		t.Fatalf("expected disconnect_reason packet before timeout close, got %v", transport.sent)
	}
}
