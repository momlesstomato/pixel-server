...
// Transport defines packet write behavior required by economy realtime runtime.
type Transport interface {
	// Send writes one encoded packet payload to one connection identifier.
	Send(string, uint16, []byte) error
}

// Runtime defines economy realm websocket packet behavior.
type Runtime struct {
	// service stores economy application behavior.
	service *economyapp.Service
...
// NewRuntime creates one economy realtime runtime instance.
func NewRuntime(service *economyapp.Service, sessions coreconnection.SessionRegistry, transport Transport, logger *zap.Logger) (*Runtime, error) {
	if service == nil {
		return nil, fmt.Errorf("economy service is required")
	}
	if sessions == nil {
