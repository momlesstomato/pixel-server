...
func TestRegistryReturnsOrderedSeedSteps(t *testing.T) {
	steps := Registry()
	if len(steps) != 21 {
		t.Fatalf("expected twenty-one seed steps, got %d", len(steps))
	}
	...
	if steps[12] == nil || steps[12].ID != "20260324_inv_01_currency_types" {
		t.Fatalf("unexpected seed step metadata at index 12")
	}
	if steps[13] == nil || steps[13].ID != "20260320_S01_catalog_pages" {
		t.Fatalf("unexpected seed step metadata at index 13")
	}
	if steps[14] == nil || steps[14].ID != "20260405_S02_hc_shop_page" {
		t.Fatalf("unexpected seed step metadata at index 14")
	}
	if steps[15] == nil || steps[15].ID != "20260320_S02_club_offers" {
		t.Fatalf("unexpected seed step metadata at index 15")
	}
	if steps[16] == nil || steps[16].ID != "20260405_S03_subscription_users" {
		t.Fatalf("unexpected seed step metadata at index 16")
	}
	if steps[17] == nil || steps[17].ID != "20260326_S01_nav_categories" {
		t.Fatalf("unexpected seed step metadata at index 17")
	}
	if steps[18] == nil || steps[18].ID != "20260326_S02_nav_demo_rooms" {
		t.Fatalf("unexpected seed step metadata at index 18")
	}
	if steps[19] == nil || steps[19].ID != "seed_20260401_01_room_models" {
		t.Fatalf("unexpected seed step metadata at index 19")
	}
	if steps[20] == nil || steps[20].ID != "20260405_09_permission_assignment_backfill" {
